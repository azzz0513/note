...
### ==Swiss Table内存布局==
```Go
// src/internal/runtime/maps/map.go (概念简化版)
type Map struct {
	// 记录当前 map 中实际存在的 Key-Value 数量
	used uint64 

	// 核心字段：还能插入多少个元素就会触发扩容或整理？
	// 它是基于总容量的 7/8 计算出来的，每次向空槽插入数据会减 1。
	growthLeft uint64 

	// 记录当前分配的总容量（必须是 2 的幂）
	capacity uint64 

	// 记录分配的 Group 数量（capacity / 8）
	groupCount uint64 
	...
	// 指向连续的控制字节数组（Metadata）
	// 每个字节代表一个槽位的状态（空、满、已删除）和哈希高 7 位
	ctrl *uint8 

	// 指向连续的数据槽位（存放实际的 Key 和 Value）
	// 内存布局是连续的 Group，每个 Group 放 8 个元素的 K/V
...
#### 扩容过程是怎么样的？
在旧版中，扩容是由 hashGrow（准备） + growWork（渐进搬运） + evacuate（实际拷贝）组成的漫长过程。
在新版的 Swiss Map 中，扩容被收敛为一个极其干脆、**一次性完成（Bulk Copy）的同步阻塞过程**。

具体的扩容流程如下：
##### 1. 触发时机检测
每次执行插入操作（Put）时，如果在对应的 Group 及其探测链中找不到相同的 Key，且当前 growthLeft == 0，就会触发扩容逻辑。

##### 2. 判断扩容类型（Grow vs Rehash）
Go 会判断当前 growthLeft == 0 到底是因为真的“满”了，还是因为“墓碑”太多了：
- **翻倍扩容（Grow）**：如果实际存在的元素数量（used）已经超过了容量的 **7/8（即 87.5%）**，说明确实装不下了，需要将 capacity 翻倍。
- **等量整理（Rehash/Compact）**：如果实际 used 的元素并不多，但 growthLeft 还是归零了，说明被**删除的墓碑（Deleted）太多了**。此时不增加总容量，只分配一块同等大小的新内存，把有效数据搬过去，顺便丢弃所有墓碑。

##### 3. 分配新内存
根据上一步计算出的新容量，直接调用内存分配器（mallocgc），申请一块全新的、连续的大内存块，用于存放新的 ctrl 数组和 groups 数组。

##### 4. 全量搬迁（核心拷贝循环）
**注意：这一步会阻塞当前的写操作，直到整个 map 搬运完毕。**

Go 底层会启动一个高效的 for 循环，遍历**旧的 ctrl 数组**：
1. 每次读出旧表中的 1 个字节（状态位）。
2. 如果状态是 Empty（空）或 Deleted（墓碑），**直接跳过**。
3. 如果状态是 Full（有数据）：
    - 从旧的 groups 内存中读取真实的 Key 和 Value。
    - 拿着这个 Key，用哈希函数重新计算在**新表**中的位置。
    - 找到新表中的目标 Group，如果冲突了就往后线性探测，找到一个空位（Empty）。
    - 将 Key、Value 写入新表的 groups，并将新表的 ctrl 对应字节设为 Full 加上哈希高 7 位。

##### 5. 替换指针与释放旧表
当旧表中所有 Full 的槽位都被搬到新表后：
- 将 Map 结构体中的 ctrl 和 groups 指针**直接指向新分配的内存**。
- 更新 capacity 和 growthLeft。
- **旧内存直接抛弃**（没有指针指向它了，Go 的垃圾回收器 GC 会在后台自行清理）。
- 扩容结束，恢复刚才被阻塞的插入操作。

#### 为什么敢全量迁移
1. **缓存极其友好（Cache Line 友好的连续内存）**
    - 旧版的渐进式搬迁，需要顺着 bmap 里的 overflow 指针全内存到处乱跳着读，这会引发极其严重的 **CPU Cache Miss（缓存未命中）**，导致搬数据非常慢。
    - 新版的连续内存布局，使得搬迁过程本质上是对一大块连续内存的**线性扫描（Linear Scan）**。现代 CPU 的硬件预取器（Prefetcher）极其擅长处理这种模式，读取速度堪比直接内存拷贝（memmove）。
2. **没有指针要处理**
    - 搬运过程只需要算哈希、读连续内存、写连续内存。没有任何链表需要拆解、没有任何指针需要重新绑定。
...
对于Swiss table：
**Swiss map 是一块完全连续的内存：**  
它把 map 扁平化为了两个连续的数组：一个 ctrl 数组（存状态和哈希高位）和一个 slots 数组（存 Key-Value）。  
因为内存是连续的，**顺序遍历和复制整块连续内存的速度极其快（得益于 CPU Cache 预取和现代 CPU 架构）**。
1. 触发时机
	- **负载因子过高**：Swiss map 的负载因子上限被提高到了 **~0.875**（旧版是 0.8125 即 6.5/8）。当装载率超过 7/8 时，触发翻倍扩容（Grow）。
	- **墓碑（Tombstone）过多**：删除元素时，Swiss map 只是在 ctrl 字节里标记一个“已删除”（墓碑）。如果不断插入删除，墓碑太多会影响查找性能，此时触发**等量整理（Rehash/Compact）**。
2. 一次性执行流程：当某个写入操作（Assign）触发扩容时：
3. **分配新内存**：直接申请一块容量为原来 2 倍（或等量）的全新连续内存（新的 ctrl 和 slots 数组）。
4. **全量搬迁（Rehash）**：
    - **阻塞当前 Goroutine**，在一个循环中，线性扫描旧的 ctrl 和 slots 数组。
    - 把有效的 Key-Value 重新计算哈希，全部插入到新数组中。
5. **释放旧内存**：搬运完毕后，直接将底层指针指向新数组，旧数组立刻被丢弃（交给 GC）。
6. **完成写入**：继续执行刚才触发扩容的那次写入操作。
