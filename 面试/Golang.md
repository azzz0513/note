...
| 操作     | 处理逻辑                            |
| ------ | ------------------------------- |
| **读**  | 先查旧 buckets，再查新 buckets         |
| **写**  | 触发迁移：将旧桶的 1-2 个 key 搬到新桶，然后写入新桶 |
| **删除** | 标记删除，扩容完成后再清理                   |
```
┌─────────┐      ┌─────────┐      ┌─────────┐
│  旧桶    │      │  旧桶    │      │  旧桶    │
...
| ---------------- | ------------------- |
| **读**            | 正常读，Table 分裂期间无影响   |
| **写导致分裂**        | 只锁定当前 Table，分裂完成后写入 |
| **其他 Table 的读写** | **完全不受影响**，无锁等待     |

```
Directory（表目录）→ 指向多个独立的 Table
                         ↓
                    某个 Table 满了 → 只分裂这个 Table
                         ↓
                    其他 Table 不受影响，继续读写
```
```
